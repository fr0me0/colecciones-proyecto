...
public class EjemploVuelo {
    ...
    public static void main(String[] args) throws ParseException {
        ...
        llegadas.sort((v1, v2) -> v2.getMomentoLlegada().compareTo(v1.getMomentoLlegada()));

        llegadas.forEach(System.out::println);

        Vuelo ultimoVuelo = llegadas.get(0);
        System.out.println("El último vuelo en llegar es " + ultimoVuelo.getNombre() + ": " + ultimoVuelo.getOrigen() + ", aterriza el " + ultimoVuelo.getMomentoLlegada());

        llegadas.sort((v1, v2) -> v2.getCantidadPasajeros().compareTo(v1.getCantidadPasajeros()));

        Vuelo vueloMenorNumero = new LinkedList<>(llegadas).peekLast();
        assert vueloMenorNumero != null;
        System.out.println("El vuelo con menor número de pasajeros es " + vueloMenorNumero.getNombre() + ": " + vueloMenorNumero.getOrigen()+ ", con " + vueloMenorNumero.getCantidadPasajeros() + " pasajeros.");
